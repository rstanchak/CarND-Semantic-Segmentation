...
def get_bilinear_filter(filter_shape, upscale_factor):
    ...
    bilinear_weights = tf.get_variable(name="decon_bilinear_filter",
                                       initializer=init,
                                       shape=weights.shape)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op,
             ...
             learning_rate, metrics=None, writer=None):
    """
...
class MyMetrics(object):
    def __init__(self, correct_label, logits, num_classes):
        ...
        with tf.name_scope("my_metrics"):
            confusion = tf.cast(tf.confusion_matrix(label, prediction),
                                tf.float32)
            TP = confusion[1, 1]
            TN = confusion[0, 0]
            FP = confusion[0, 1]
